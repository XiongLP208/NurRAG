...
def knowledge_base_page(api: ApiRequest):
    ...
    if selected_kb == "New Knowledge Base":
        ...
        # 上传文件
        files = st.file_uploader("Uploading knowledge files：",
        ...
        with st.expander(
            ...
            cols = st.columns(3)
            chunk_size = cols[0].number_input("Maximum length of a single paragraph：", 1, 1000, CHUNK_SIZE)
        ...
        if not len(doc_details):
            st.info(f"Knowledge Base `{kb}` no files")
        else:
            st.write(f"Knowledge Base `{kb}` has files:")
            st.info("The Knowledge Base contains source files and vector libraries, please select the files from the table below and then operate them")
            ...
            # 将文件分词并加载到向量库中
            if cols[1].button(
                    "Re-add to vector library" if selected_rows and (pd.DataFrame(selected_rows)["in_db"]).any() else "Add to vector library",
        ...
        if cols[0].button(
                "Rebuild vector library from source files",
                # help="无需上传文件，通过其它方式将文档拷贝到对应知识库content目录下，点击本按钮即可重建知识库。",
                use_container_width=True,
                type="primary",
        ):
            with st.spinner("Please be patient and do not refresh or close the page while the vector library is being reconstructed."):
                empty = st.empty()
                empty.progress(0.0, "")
